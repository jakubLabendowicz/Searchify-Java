...
const Main = () => {
    ...
    const handlePostTrack = async (track) => {
        try {
            let token = localStorage.getItem("token")
            const url = "http://localhost:8081/api/tracks/"
            const { data: res } = await axios.post(url, track, {
                headers: {
                  Authorization: `Bearer ${token}`
                }
            })
    ...
    const handleDeleteTrack = async (id) => {
        try {
            let token = localStorage.getItem("token")
            const url = "http://localhost:8081/api/tracks/" + id
    ...
    const handlePutTrack = async (track) => {
        try {
            console.log(track);
            let token = localStorage.getItem("token")
            const url = "http://localhost:8081/api/tracks/" + track.id
            const { data: res } = await axios.put(url, track, {
                headers: {
                  Authorization: `Bearer ${token}`
                }
            })
    ...
    const handlePostSelfFavorite = async (id) => {
        try {
            let token = localStorage.getItem("token")
            const url = "http://localhost:8081/api/users/self/favorites/" + id
                                ...
                                sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                    ...
                                    <TableCell align="right">
                                        <Stack direction="row" alignItems="center" spacing={1}>
                                            ...
                                            {row.favorite === true ? 
                                            <IconButton aria-label="star" size="small" onClick={()=>{handleDeleteSelfFavorite(row.id)}}>
                                                <StarIcon />
                                            </IconButton>         
                                            :
                                            <IconButton aria-label="star" size="small" onClick={()=>{handlePostSelfFavorite(row.id)}}>
                                                <StarHalfIcon />
                                            </IconButton>
                                            }
                                            ...
                                            <IconButton aria-label="delete" size="small" onClick={()=>{handleDeleteTrack(row.id)}}>
                                                <DeleteIcon />
                                            </IconButton>
                                        </Stack>
                                    </TableCell>
